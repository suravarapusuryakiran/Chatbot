    ...
    for entity in filtered_entities:
        ...
        for row in res_file_info["rows"]:
            ...
            if skip_download_file(row):
                ...
                shutil.copy(fullpath, "/mnt/c/work/usu1lo/spyderPro/customer_files/")
                ts = time.time()
                os.utime("/mnt/c/work/usu1lo/spyderPro/customer_files/"+fname,(ts,ts))
                available = True
                break
...
def rasa_get_file(material_number):
    ...
    if len(entities):
        ...
        return False,"Material number not found in windchill" 
